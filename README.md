...
## whatsapp-web.js
Located in `src/main.ts`, this is the headless browser that connects to WhatsApp. For every message either 1-1 or group messages, it logs to `logs/messages.log`. The formate is:
...
## summerizer
`summerizer/summerize.py` loads the model from `model`, reads the previous day's log file (`logs/messages.log.0`), compiles a list of important messages and emails using SMTP. 
